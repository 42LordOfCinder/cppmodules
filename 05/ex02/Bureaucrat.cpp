...
void	Bureaucrat::executeForm(AForm const &fr) const {
	try {
		fr.execute(*this);
	}
	catch (std::exception &e) {
		std::cerr << this->name << " couldn't execute " << fr.getName() << " because " << e.what() << std::endl;
		return ;
	}
	std::cout << this->name << " executed " << fr.getName() << std::endl;
}
